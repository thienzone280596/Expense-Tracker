...
//  ExpenseTracker
//
//  Created by ThienTran on 19/8/25.
//
...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
  ...
  func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
    guard let windowScene = (scene as? UIWindowScene) else { return }
     ...
     // Seed Category khi app lần đầu chạy
     if !UserDefaults.standard.bool(forKey: "didSeedCategories") {
  ...
  private func setInitialRoot() {
         if SessionManager.shared.isLoggedIn {
             let main = MainTabBarController() // hoặc MainTabBarController
